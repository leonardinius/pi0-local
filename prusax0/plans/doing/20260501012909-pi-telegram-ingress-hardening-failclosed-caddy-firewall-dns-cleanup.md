...
- [x] Step 1: Зафиксировать ingress security contract и scope
  - Updated `plans/doing/projpub-v1-cf-caddy-localbind.md` with explicit ingress contract: hostname and managed DNS only `<project>-<base>`, `APP_PUBLIC_URL` forbidden as host override, and non-goals clarified.
- [x] Step 2: Сконсолидировать fail-closed validation в одном источнике истины
  - Added shared validator `scripts/expose-validation.ts` reused by `scripts/validate-expose.ts` and `scripts/sync-ingress.ts`; includes `INVALID_PROJECT_SLUG` and explicit loopback APP_PORT compose contract checks.
- [x] Step 3: Переподключить `sync-ingress` на validation matrix и reason summary
  - `sync-ingress` now generates routes only from valid matrix entries and prints stable JSONL summary (`ingress_validation_summary`) by reason codes while preserving validate/reload/rollback flow.
